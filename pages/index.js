    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .card {
      background: linear-gradient(145deg,
        ...
        rgba(180,19,60,.12) 60%,
        rgba(255,255,255,.04) 100%);
      border: 1px solid rgba(244,63,108,.3);
      border-radius: 2rem;
      max-width: 680px;
      width: 100%;
      ...
      box-shadow:
        0 0 60px rgba(244,63,108,.18),
        0 0 120px rgba(180,19,60,.10),
    ...
    .pill {
      background: rgba(244,63,108,.15);
      border: 1px solid rgba(244,63,108,.35);
      border-radius: 999px;
    ...
    .btn-love {
      ...
      font-style: italic;
      color: #fff;
      background: linear-gradient(135deg, #f43f6c, #b5133c);
      border: none;
      border-radius: 999px;
      cursor: pointer;
      position: relative;
      overflow: hidden;
      box-shadow: 0 4px 30px rgba(244,63,108,.5);
    ...
    .btn-love:hover::before { opacity: 1; }
    ...
    .burst {
      ...
      z-index: 99;
      font-size: 1.4rem;
      animation: burst-anim .8s ease-out forwards;
    }
...
<main>
  ...
  <div class="card reveal">
    ...
    <div class="pills">
      <span class="pill">🍬 minha paçoquinha</span>
      <span class="pill">👸 minha princesa</span>
      <span class="pill">💖 meu amor</span>
      <span class="pill">✨ minha vida</span>
      <span class="pill">🌹 meu tudo</span>
    </div>

    <button class="btn-love" id="btn">💌 Clique para sentir meu amor</button>
  </div>

  <p class="footer-note reveal">feito com todo o amor do mundo só pra você 🌹</p>
</main>
...
<script>
  ...
  function drawStars(t) {
    ...
    stars.forEach(s => {
      const alpha = .3 + .7 * Math.abs(Math.sin(t * s.speed + s.phase));
      ctx.beginPath();
  ...
  document.addEventListener('mousemove', e => {
    if (Math.random() > .88) spawnSparkle(e.clientX, e.clientY);
  });
  ...
  btn.addEventListener('click', e => {
    e.stopPropagation(); // burst still fires via document
